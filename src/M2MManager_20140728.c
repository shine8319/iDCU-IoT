...
int Pack_Manager( unsigned char* buffer, int len ) 
{
	...
	double	tm_year, tm_month;
	double	tm_day, tm_hour, tm_min, tm_sec;
	double d_diff;
	struct tm t;
	char syncError;
	...
	*/
	...
	switch( type )
	{
		...
		case GETDATA:
			...
			/*
			printf(" d_diff = %d\n", d_diff );
			tm_year   = d_diff / (double)( ( 60 *60 * 24) * 365 );

			tm_day   = d_diff / (double)( 60 *60 * 24);
			d_diff   = d_diff - ( tm_day *(double)60 *(double)60 *(double)24);
			printf(" d_diff = %d\n", d_diff );

			tm_hour  = d_diff / (double)( 60 *60);
			d_diff   = d_diff - ( tm_hour *(double)60 *(double)60);
			printf(" d_diff = %d\n", d_diff );

			tm_min   = d_diff / (double)60;
			d_diff   = d_diff - ( tm_min *(double)60);
			printf(" d_diff = %d\n", d_diff );

			tm_sec   = d_diff;

			printf( "%d year %dday %dhour %dmin %dsec\n", 
					tm_year,
