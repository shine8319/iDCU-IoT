...
static int Socket_Manager( int *client_sock ) {
    ...
    writeLog( "/work/smart/comm/log/PLCS9", "[PLCS9] start");
	...
	if( nd > 0 ) 
	{
		...
		if( receiveSize >= BUFFER_SIZE*10 )
		{
		    ...
		    writeLog( "/work/smart/comm/log/PLCS9", "[PLCS9] Packet Buffer Full~~");
		...
		sleep(1);
		printf("receive None\n");
		writeLog( "/work/smart/comm/log/PLCS9", "[PLCS9] receive None");
		break;
	    }
	    ReadMsgSize = 0;
	...
	{
	    printf("timeout\n");
	    writeLog( "/work/smart/comm/log/PLCS9", "[PLCS9] timeout");
	...
	{
	    printf("error...................\n");
	    writeLog( "/work/smart/comm/log/PLCS9", "[PLCS9] network error........");
	    //shutdown( *client_sock, SHUT_WR );
	    break;
	}
	nd = 0;
    ...
    writeLog( "/work/smart/comm/log/PLCS9", "[PLCS9] Disconnection ");
...
static int selectTag(unsigned char* buffer, int trCount, int len )
{
	...
	writeLog("/work/smart/comm/log/PLCS9", "[PLCS9] msgsnd() error : Queue full" );
	//sleep(1);
	//return -1;
    }
