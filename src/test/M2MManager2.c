...
int getNodeStatusData( QUERYTIME time ) {
	...
	query = sqlite3_mprintf("select nodeid, 6, port1, port2, port3, port4, port5, port6, strftime('%%Y%%m%%d%%H%%M%%S', datetime) from tb_comm_status");
	// add strftime('%%Y%%m%%d%%H%%M%%S', datetime)
...
#define SENDBUFF	16	
	int rtrn;
	int bigDataLength = (totalLength+4);
	int divideCnt = bigDataLength / SENDBUFF;
	int remainLength = bigDataLength % SENDBUFF;
	printf("bigDataLength %d, divideCnt %d, remainLength %d\n",
			bigDataLength,
			divideCnt,
			remainLength);

	int bigDataOffset = 0;
	if( bigDataLength >= 4096 )
	{
		if( divideCnt > 0 )
		{
			printf("Big Data!!!!!!!\n");
			do {
				...
				rtrn = write(wifi, tlvSendBuffer+bigDataOffset, SENDBUFF);
				//printf("first send Length %d\n", rtrn);
				bigDataOffset += SENDBUFF;
				usleep(1000*10);
				//sleep(2);
				//printf("divideCnt %d, bigDataOffset %d\n", divideCnt, bigDataOffset );
				divideCnt--;;

			}while( divideCnt > 0 );

			rtrn = write(wifi, tlvSendBuffer+bigDataOffset, remainLength);
			printf("second send Length %d\n", rtrn);
		}
		else
		{
			rtrn = write(wifi, tlvSendBuffer+bigDataOffset, remainLength);
		...
		for( i = bigDataOffset+remainLength-4; i < bigDataOffset+remainLength; i++ )
		{
			printf("%02X ", tlvSendBuffer[i] );
		}
		printf("\n");
	...
	{
		rtrn = write(wifi, tlvSendBuffer, bigDataLength);
	}

	printf("Send rtrn = %d | totalLength+4 = %d\n", rtrn, totalLength+4);
