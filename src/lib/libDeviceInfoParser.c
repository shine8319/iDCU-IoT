...
{ 
	...
	{
	    if (strcmp(current_el,"DRIVER") == 0) 
	    {
		sprintf(devInfo.device[user_num-1].driver, "%s",buf);
	    }
	    else if (strcmp(current_el,"DEVICEID") == 0) 
	    {
		sprintf(devInfo.device[user_num-1].id, "%s",buf);
	    }
...
static void start(void *data, const char *el, const char **attr) { 
    ...
    if (strcmp(current_el,"DEVICEINFO") == 0) 
    { 
	//printf("[%d] %s \n", user_num, current_el); 
	user_num++; 
    } 
