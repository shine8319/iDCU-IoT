...
{ 
	...
	else 
	{
	    ...
	    if (strcmp(current_el,"ADDRESS") == 0) 
	    {
		sprintf(xmlinfo.address, "%s",buf);
		printf("%s\n", xmlinfo.address);
	    }
	    else if (strcmp(current_el,"CYCLE") == 0) 
	    {
		sprintf(xmlinfo.cycle, "%s",buf);
		printf("%s\n", xmlinfo.cycle);
	    }
	}
...
TIMESYNCINFO timeSyncInfoParser(const char *path) { 
...
int getTimeSyncCycle(char *data)
{
    int returnData= 5;

    if (strcmp(data,"runMinute") == 0) 
    {
	returnData = 0;
    }
    else if (strcmp(data,"run30Minute") == 0) 
    {
	returnData = 1;
    }
    else if (strcmp(data,"runHour") == 0) 
    {
	returnData = 2;
    }
    else if (strcmp(data,"run6Hour") == 0) 
    {
	returnData = 3;
    }
    else if (strcmp(data,"run12Hour") == 0) 
    {
	returnData = 4;
    }
    else if (strcmp(data,"runDay") == 0) 
    {
	returnData = 5;
    }
...
/*
UINT32 getTimeSyncAddress(char *data)
{

    UINT32 returnData;
    
    returnData = inet_addr( data );
}
*/
