...
{ 
	...
	{
	    if (strcmp(current_el,"NAME") == 0) 
	    {
		sprintf(devInfo.driver[user_num-1].name, "%s",buf);
	    }
	    else if (strcmp(current_el,"ID") == 0) 
	    {
		sprintf(devInfo.driver[user_num-1].id, "%s",buf);
	    }
...
static void start(void *data, const char *el, const char **attr) { 
    ...
    if (strcmp(current_el,"DRIVERINFO") == 0) 
    { 
	//printf("[%d] %s \n", user_num, current_el); 
	user_num++; 
    } 
...
DRIVERLIST driverInfoParser(const char *path) { 
